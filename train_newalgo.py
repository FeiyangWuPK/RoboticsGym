...
def train():
    ...
    eval_callback = EvalCallback(eval_env, 
                                 ...
                                 deterministic=True, 
                                 render=False)
    eval_student_callback = EvalStudentCallback(eval_env,
                                                best_model_save_path=f"./logs/{run.name}/student/",
                                                log_path=f"./logs/{run.name}/student/", 
                                                eval_freq=2000,
                                                n_eval_episodes=5,
                                                deterministic=True, 
                                                render=False)
	...
	# Init model
    irl_model = HIP(policy='MlpPolicy', 
    ...
    # Evaluation
    mean_reward, _ = evaluate_policy(irl_model, eval_env, n_eval_episodes=10)
    mean_student_reward, _ = evaluate_student_policy(irl_model, eval_env, n_eval_episodes=10)
    
    # Finish wandb run
    run.finish()

    return mean_reward, mean_student_reward
