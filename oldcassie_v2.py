...
from old_cassie.cassie_env.quaternion_function import euler2quat, inverse_quaternion, quaternion_product, quaternion2euler, rotate_by_quaternion, quat2yaw
from old_cassie.cassie_m.cassiemujoco import pd_in_t, state_out_t, CassieSim, CassieVis
...
class OldCassieMirrorEnv(gym.Env, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.sim = CassieSim(self.model_xml_path)
        self.visual = visual
        ...
        self.trajectory = CassieTrajectory(
            "old_cassie/trajectory/stepdata.bin")
    ...
    def get_kin_state(self):
        if self.speed < 0:
            ...
            phase = self.phase
            pose = np.copy(self.backward_trajectory[phase*self.control_rate][0]) * interpolate + (
                1 - interpolate) * np.copy(self.step_in_place_trajectory[phase*self.control_rate][0])
            pose[0] += (self.backward_trajectory[(self.max_phase-1)*self.control_rate]
                        [0][0] - self.backward_trajectory[0][0][0]) * self.counter * (-self.speed)
            pose[1] = 0
            vel = np.copy(self.backward_trajectory[phase*self.control_rate][1])
            vel[0] *= (-self.speed)
        elif self.speed <= 1.0:
            interpolate = self.speed * 1
            pose = np.copy(self.trajectory.qpos[self.phase*self.control_rate]) * interpolate + (
                1 - interpolate) * np.copy(self.step_in_place_trajectory[self.phase*self.control_rate][0])
            pose[0] = self.trajectory.qpos[self.phase*self.control_rate, 0]
            pose[0] *= self.speed
            pose[0] += (self.trajectory.qpos[1681, 0] -
            ...
            vel[0] *= self.speed
        # print("vel", vel[0])
        pose[1] = self.side_speed * \
            (self.counter*self.max_phase+self.phase)*self.control_rate*0.0005
        pose[3] = 1
        pose[4:7] = 0
        pose[7] = 0
        pose[8] = 0
    ...
    def get_kin_next_state(self):
        if self.speed < 0:
            ...
            pose[0] += (self.trajectory.qpos[1681, 0] -
                        self.trajectory.qpos[0, 0]) * counter * self.speed
            # print(pose[0])
            pose[1] = 0
            vel[0] *= self.speed
            pose[1] = self.side_speed * \
                (counter*self.max_phase+self.phase)*self.control_rate*0.0005
    ...
    def reset(self, seed=None, options=None):
        ...
        self.sim.c = cassie_sim_init(
            self.model_xml_path.encode('utf-8'), False)
    ...
    def _get_obs(self):
        if len(self.state_buffer) > 0 and self.random_state:
        ...
        if self.phase < 14:
            ...
            useful_state = np.copy(np.concatenate([[state.pelvis.position[2] - state.terrain.height], new_orientation[:], state.motor.position[:], new_translationalVelocity[:],
                                   state.pelvis.rotationalVelocity[:], state.motor.velocity[:], new_translationalAcceleration[:], state.joint.position[:], state.joint.velocity[:]]))
            ...
            pelvis_euler = quaternion2euler(
                np.copy(state.pelvis.orientation[:]))
            ...
            translational_velocity = np.copy(
            ...
            new_orientation = quaternion_product(
                iquaternion, pelvis_orientation)
            new_translationalVelocity = rotate_by_quaternion(
            ...
            useful_state = np.copy(np.concatenate([[state.pelvis.position[2] - state.terrain.height], new_orientation[:], motor_position,
                                   new_translationalVelocity[:], rotational_velocity, motor_velocity, new_translationalAcceleration[:], joint_position, joint_velocity]))
    ...
    def compute_reward(self,):
        ...
        for i in range(10):
            error = weight[i] * (ref_pos[joint_index[i]] -
                                 self.sim.qpos()[joint_index[i]])**2
            joint_penalty += error*30
        pelvis_pos = np.copy(self.cassie_state.pelvis.position[:])
        com_penalty = (pelvis_pos[0]-ref_pos[0])**2 + \
            (pelvis_pos[1]-ref_pos[1])**2 + (self.sim.qvel()[2])**2
        yaw = quat2yaw(self.sim.qpos()[3:7])
        orientation_penalty = (
            self.sim.qpos()[4])**2+(self.sim.qpos()[5])**2+(yaw - self.orientation)**2
        spring_penalty = (self.sim.qpos()[15])**2+(self.sim.qpos()[29])**2
        spring_penalty *= 1000
        total_reward = 0.5*np.exp(-joint_penalty)+0.3*np.exp(-com_penalty) + \
            0.2*np.exp(-10*orientation_penalty)  # +0.1*np.exp(-spring_penalty)
    ...
    def step(self, action):
        ...
        self.useful_recorded_data.append([left_foot_force, right_foot_force] + center_of_mass_pos +
                                         feet_pos + center_of_mass_vel + center_of_mass_angular_momentum + center_of_mass_centroid_inertia)
        ...
        done = not (height > 0.4 and height <
                    100.0) or self.time >= self.time_limit
        yaw = quat2yaw(self.sim.qpos()[3:7])
        if self.visual:
            self.render()
        ...
        if done and self.record_for_reward_inference:
            ...
            os.makedirs("logs/reward_inference/traj_state_buffer",
                        exist_ok=True)
            ...
            os.makedirs(
                "logs/reward_inference/traj_useful_recorded_data", exist_ok=True)
