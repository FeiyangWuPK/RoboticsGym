...
import warnings
import numpy as np
import roboticsgym.envs.oldcassie_v0
...
def obtain_ik_data_in_replay_buffer(n_traj: int = 10):
    env = make_vec_env(
        "CassieMirror-v0",
        n_envs=1,
    )
    model = SAC(policy="MlpPolicy", env=env, verbose=1, train_freq=600 * n_traj)
    _, callback = model._setup_learn(
        600 * n_traj,
        callback=None,
    )
    ...
    model.save_replay_buffer("expert_trajectories/ik/10traj_ik")
