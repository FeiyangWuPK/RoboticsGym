...
class cassieRLEnvMirror(cassieRLEnvMultiTraj):
    ...
    def step(self, action):
        ...
        center_of_mass_centroid_inertia = (
            self.sim.centroid_inertia()
        )  # 3 by 3 matrix, list of 9
        feet_pos = self.sim.foot_pos()
        sa_pair = np.concatenate([self.get_state(), action])
        ...
        self.useful_recorded_data.append(sa_pair)
        ...
        if done and self.record_for_reward_inference:
            ...
            with open(
                f"logs/reward_inference/traj_state_buffer/buffer_{t}_{self.reward_file_counter}",
                "wb",
            ) as fp:
            ...
            with open(
                f"logs/reward_inference/traj_useful_recorded_data/buffer_{t}_{self.reward_file_counter}",
                "wb",
            ) as fp2:
                pickle.dump(self.useful_recorded_data, fp2)
            self.useful_recorded_data = []

            self.reward_file_counter += 1
