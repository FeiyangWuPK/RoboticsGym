...
class OldCassieMirrorEnv(gym.Env, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model_xml_path = "arm_cassie_env/cassie_m/model/0cassie.xml"
        self.random_terrain = random_terrain
        if self.random_terrain:
            print("random terrain")
            self.model_xml_path = "arm_cassie_env/cassie_m/model/cassie_hfield.xml"
        print(self.model_xml_path)
        self.sim = CassieSim(self.model_xml_path, terrain=True)
        self.visual = visual
    ...
    def step(self, action):
        ...
        if done and self.record_for_reward_inference:
            ...
            with open(
                f"logs/reward_inference/traj_state_buffer/buffer_{t}", "wb"
            ) as fp:
            ...
            with open(
                f"logs/reward_inference/traj_useful_recorded_data/buffer_{t}", "wb"
            ) as fp2:
