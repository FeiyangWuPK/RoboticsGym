from roboticsgym.envs.old_cassie.cassie_env.cassieRLEnvMirror import cassieRLEnvMirror
...
class OldCassieMirrorEnv(gym.Env, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "render" in kwargs:
            ...
            self.visual = False
        self.env = cassieRLEnvMirror(visual=self.visual)

        # self.env.vis.init_recording(f"logs/videos/IPMD")
        # self.env.vis.set_cam("cassie-pelvis", 3, 90, -20)
    ...
    def render(self):
        draw_state = self.env.vis.draw(self.env.sim)
        self.env.vis.record_frame()
        return draw_state
