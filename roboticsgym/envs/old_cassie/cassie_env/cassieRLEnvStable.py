...
from roboticsgym.envs.old_cassie.cassie_env.cassieRLEnvMirrorIKTraj import *
...
class cassieRLEnvStable(cassieRLEnvMirror):
    def __init__(self, visual=False, setforce=-1, fix=True):
        ...
        self.action_space = spaces.Box(
            low=np.float32(np.array([-3.14 / 3] * 10)),
            high=np.float32(np.array([3.14 / 3] * 10)),
        )
    ...
    def step(self, action):
        ...
        if self.acbuf:
            ...
            if len(self.action_buf) > 3:
                ...
                action = (
                    self.action_buf[0] * 0.1
                    + self.action_buf[1] * 0.2
                    + self.action_buf[2] * 0.7
                )
    ...
    def compute_reward(self, action):
        ...
        if len(self.prev_action) > 1:
            hip_action_penalty = np.linalg.norm(
                action[[0, 5]] - self.prev_action[[0, 5]]
            )
        else:
        ...
        orientation_penalty = (
            roll**2 + pitch**2 + (yaw - self.orientation) ** 2
        )  # wqwqwq
