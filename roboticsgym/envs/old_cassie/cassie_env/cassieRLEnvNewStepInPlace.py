from roboticsgym.envs.old_cassie.cassie_env.cassieRLEnvWithMoreState import *
...
class cassieRLEnvNewStepInPlace(cassieRLEnvWithFootForces):
    def __init__(self):
        ...
        self.trajectory = CassieTrajectory(
            "arm_cassie_env/trajectory/more-poses-trial.bin"
        )
        self.policy_freq = 58
        self.max_phase = 29
        ...
        useful_state = np.copy(
            np.concatenate(
                [
                    ...
                    state.leftFoot.toeForce[:],
                    state.leftFoot.heelForce[:],
                    state.rightFoot.toeForce[:],
                    state.rightFoot.heelForce[:],
                ]
            )
        )
    ...
    def get_mirror_state(self, state):
        ...
        return np.concatenate(
            [
                np.copy(orientation),
                np.copy(height),
                np.copy(pelvis_orientation),
                np.copy(right_motor_p),
                np.copy(left_motor_p),
                np.copy(pelvis_vel),
                np.copy(pelvis_rot_vel),
                np.copy(right_motor_v),
                np.copy(left_motor_v),
                np.copy(pelvis_acc),
                np.copy(right_toe_force),
                np.copy(right_heel_force),
                np.copy(left_toe_force),
                np.copy(left_heel_force),
                np.copy(ref_pelvis_p),
                np.copy(ref_right_motor_p),
                np.copy(ref_left_motor_p),
                np.copy(ref_vel_p),
                np.copy(ref_right_motor_v),
                np.copy(ref_left_motor_v),
            ]
        )
    ...
    def compute_reward(self):
        ...
        com_penalty = (
            (pelvis_pos[0]) ** 2
            + (pelvis_pos[1]) ** 2
            + (pelvis_pos[2] - ref_pos[2]) ** 2
        )

        yaw = quat2yaw(self.sim.qpos()[3:7])

        orientation_penalty = (
            (self.sim.qpos()[4] * 20) ** 2
            + (self.sim.qpos()[5] * 20) ** 2
    ...
    def reset(self):
        self.orientation = (
            0 * np.pi
        )  # 0.5*np.pi#0.5*np.pi#random.randint(-4, 4) * np.pi / 4.0
