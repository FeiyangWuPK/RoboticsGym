from roboticsgym.envs.old_cassie.cassie_env.cassieRLEnv import *
...
class cassieRLEnvStepInPlaceWithFootForces(cassieRLEnvDelay):
    def __init__(self, visual):
        ...
        self.action_space = spaces.Box(
            low=np.float32(np.array([-3.14 / 6] * 10)),
            high=np.float32(np.array([3.14 / 6] * 10)),
        )
...
class cassieRLEnvWithFootForces(cassieRLEnvStepInPlaceWithFootForces):
    ...
    def compute_reward(self):
        ...
        com_penalty = (
            (pelvis_pos[0] - desired_x) ** 2
            + (pelvis_pos[1] - desired_y) ** 2
            + (pelvis_pos[2] - ref_pos[2]) ** 2
        )
        ...
        total_reward = (
            0.5 * np.exp(-joint_penalty)
            + 0.3 * np.exp(-com_penalty)
            + 0.1 * np.exp(-30 * orientation_penalty)
