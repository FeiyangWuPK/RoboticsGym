...
class cassieRLEnv:
    def __init__(self, visual):
        self.model = "/home/feiyang/Develop/Cassie/arm-cassie/arm_cassie_env/cassie_m/model/0cassie.xml"
    ...
    def compute_reward(self):
        ...
        com_penalty = (
            (ref_pos[0] - self.sim.qpos()[0]) ** 2
            + (self.sim.qpos()[1]) ** 2
            + (self.sim.qpos()[2] - ref_pos[2]) ** 2
        )
...
class cassieRLEnvDelay(cassieRLEnv):
    def __init__(self, visual):
        self.model = "./arm_cassie_env/cassie_m/model/0cassie.xml"
        self.sim = CassieSim(self.model)
        self.visual = visual
        if self.visual:
            self.vis = CassieVis(self.sim)
        ...
        self.action_space = spaces.Box(
            low=np.float32(np.array([-1] * 10)), high=np.float32(np.array([1] * 10))
        )
        self.trajectory = CassieTrajectory("arm_cassie_env/trajectory/stepdata.bin")
        self.P = np.array([100, 100, 88, 96, 50, 100, 100, 88, 96, 50])
        self.D = np.array([10.0, 10.0, 8.0, 9.6, 5.0, 10.0, 10.0, 8.0, 9.6, 5.0])
