...
from arm_cassie_env.cassie_m.cassiemujoco_ctypes import cassie_sim_init, cassie_sim_free
...
class OldCassieMirrorEnv(gym.Env, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.trajectory = CassieTrajectory("arm_cassie_env/trajectory/stepdata.bin")

        self.atomic_reward = SimpleNN(119, 20)

        self.atomic_reward.load_state_dict(
            th.load(
                "/home/feiyang/Research/Robotics/arm-cassie_copy/atomic_reward_model.pth"
            )
        )
        self.x_vel_sum = 0
        with open("arm_cassie_env/trajectory/stepping_trajectory_Nov", "rb") as fp:
            self.step_in_place_trajectory = pickle.load(fp)
        with open("arm_cassie_env/trajectory/backward_trajectory_Nov", "rb") as fp:
        ...
        self.linear_reg = pickle.load(open("finalized_model.sav", "rb"))
    ...
    def compute_enhanced_reward(self):
        ...
        # additional reward from reward estimation model learned from expert data
        # Aug14: remove reward_est into custom SAC model

        # return forward_reward - ctrl_cost + alive_reward + reward_est
        # Aug 14: less reward from forward motion. Should be more stable
        # return 0.5 * forward_reward - ctrl_cost + alive_reward
        # Aug 14: less reward from forward motion. Should be more stable
        return 0.5 * forward_reward - ctrl_cost + alive_reward
    ...
    #     feature = construct_features(
    #         pelvis_position=self.sim.qpos()[:3],
    #         pelvis_orientation=self.sim.qpos()[3:7],
    #         center_of_mass=self.sim.center_of_mass_position(),
    #         left_foot_forces=left_foot_force,
    #         right_foot_forces=right_foot_force,
    #         foot_pos=self.sim.foot_pos(),
    #         action=self.current_action,
    #         state=np.array(self.cassie_state),
    #         com_am=self.sim.angular_momentum(),
    #         com_inertia=self.sim.centroid_inertia(),
    #         com_vel=self.sim.center_of_mass_velocity(),
    #     )
    #     atomic_reward = self.atomic_reward.predict(feature.reshape(1, -1))[0]
    #     return 0.5 * forward_reward - ctrl_cost + alive_reward + atomic_reward

    def compute_enhanced_reward_w_linear_term(self):
        com_vel = self.sim.center_of_mass_velocity()
        x_vel = com_vel[0]
        vel_index = [6, 7, 8, 12, 18, 19, 20, 21, 25, 31]
        forward_reward = 0.25 * x_vel
        ...
        #     pelvis_orientation=self.sim.qpos()[3:7],
        #     center_of_mass=self.sim.center_of_mass_position(),
        #     left_foot_forces=self.sim.left_foot_force(),
        #     right_foot_forces=self.sim.right_foot_force(),
        #     foot_pos=self.sim.foot_pos(),
        #     action=self.current_action,
        #     state=self._get_obs(),
        #     com_am=self.sim.angular_momentum(),
        ...
        #     com_vel=self.sim.center_of_mass_velocity(),
        # )
        # linear_reward = 2 * self.linear_reg.predict(feature.reshape(1, -1))[0]

        ref_pos, ref_vel = self.get_kin_state()
        orientation_penalty = np.exp(
            -np.linalg.norm(self.sim.qpos()[3:7] - ref_pos[3:7]) ** 2
        )

        pitch = quaternion2euler(self.sim.qpos()[3:7])[1]
        ref_pitch = quaternion2euler(ref_pos[3:7])[1]
        pitch_penalty = np.exp(-np.abs(pitch - ref_pitch) ** 2)

        velocity_penalty = (
            np.exp(
                -np.linalg.norm(self.sim.qvel()[vel_index] - ref_vel[vel_index]) ** 2
            )
            # np.exp(-((self.sim.qvel()[2]) ** 2))
        )
    ...
    def compute_reward_nitpicking(self):
        com_vel = self.sim.center_of_mass_velocity()
        x_vel = com_vel[0]
        ref_pos, ref_vel = self.get_kin_state()
    ...
    def step(self, action):
        ...
        if self.enhance_reward == "enhanced":
            reward = self.compute_enhanced_reward()
        elif self.enhance_reward == "naive":
            reward = self.compute_naive_reward()
        elif self.enhance_reward == "enhanced_w_linear_terms":
            reward = self.compute_enhanced_reward_w_linear_term()
        elif self.enhance_reward == "nitpicking":
            reward = self.compute_reward_nitpicking()
        # print(reward)
        # if reward < 0.05:
        #     done = True
        # if done:
        #     # print(self.x_vel_sum / 600)
        #     # self.x_vel_sum = 0
        #     print(center_of_mass_pos[0])
...
def construct_features(
    ...
    # 6th pelvis pitch
    qpitch = pelvis_orientation[:, 1].reshape(-1, 1)
